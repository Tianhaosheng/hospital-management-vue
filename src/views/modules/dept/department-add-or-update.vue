<template>
  <el-dialog
    ...
    <el-form-item label="部门名" prop="departmentName">
      <el-input v-model="dataForm.departmentName" placeholder="部门名"></el-input>
    </el-form-item>
    <el-form-item label="部门负责人" prop="departmentPrincipal">
      <el-input v-model="dataForm.departmentPrincipal" placeholder="部门负责人"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/dept/department/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.departmentName = data.department.departmentName
                this.dataForm.departmentPrincipal = data.department.departmentPrincipal
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/dept/department/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
