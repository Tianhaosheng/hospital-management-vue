  ...
  >
    ...
    >
      ...
      <el-form-item label="上级" prop="positionSuperiorId">
        ...
        <el-select v-model="dataForm.positionSuperiorId" clearable  placeholder="请选择">
          <el-option
      ...
      <el-form-item label="职位描述" prop="positionDescription">
        <el-input
          v-model="dataForm.positionDescription"
          placeholder="职位描述"
        ></el-input>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    init(id) {
      ...
      //   ],
      this.$http({
        url: this.$http.adornUrl("/personnel/position/optionsList"),
        method: "get",
      }).then(({ data }) => {
        if (data && data.code === 0) {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(
              `/personnel/position/info/${this.dataForm.id}`
            ),
            method: "get",
            ...
            if (data && data.code === 0) {
              ...
              this.dataForm.positionSuperiorId =
                data.position.positionSuperiorId;
              this.dataForm.positionDescription =
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate((valid) => {
        console.log(this.dataForm.positionSuperiorId)
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/personnel/position/${!this.dataForm.id ? "save" : "update"}`
            ),
            method: "post",
